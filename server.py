...
app = Flask(__name__)
# Критично для Railway: доверяем прокси-заголовкам
...
# Включаем логи Engine.IO для диагностики (потом можно убрать)
socketio = SocketIO(
    app,
    ...
    while True:
        ...
        for code, room in list(active_rooms.items()):
            ...
            for sid, p in room.players.items():
                ...
                if p.keys['left']:
                    p.vx = -SPEED; p.isFacingRight = False
                elif p.keys['right']:
                    p.vx = SPEED; p.isFacingRight = True
                else:
                    p.vx = 0
            ...
            if room.mode == "PvE":
                if len(room.zombies)==0 and room.zombies_to_spawn==0:
                    room.wave += 1; room.zombies_to_spawn = room.wave * 3
                ...
                for zid, z in list(room.zombies.items()):
                    ...
                    nearest_p = None; min_dist = 9999
                    for sid, p in room.players.items():
                        if p.hp > 0:
                            dist = abs(p.x-z.x)+abs(p.y-z.y)
                            if dist < min_dist: min_dist = dist; nearest_p = p
                    if nearest_p:
                        if z.x < nearest_p.x-40: z.vx = ZOMBIE_SPEED; z.isFacingRight = True
                        elif z.x > nearest_p.x+40: z.vx = -ZOMBIE_SPEED; z.isFacingRight = False
                        else:
                            z.vx = 0
                            if z.attackTimer==0 and abs(z.y-nearest_p.y)<80:
                                nearest_p.hp -= 15; z.attackTimer = 40
                                if nearest_p.hp < 0: nearest_p.hp = 0
                    else: z.vx = 0

                    z.vy += GRAVITY; z.x += z.vx; z.y += z.vy
                    if z.vy > 0:
                        for plat in platforms:
            ...
            # Broadcast state
            state = {
                'players': {sid: {'x': p.x,'y': p.y,'hp': p.hp,'isFacingRight': p.isFacingRight,
                                  'isAttacking': p.attackTimer>0,'username': p.username,'color': p.color,
                                  'coins': p.coins} for sid,p in room.players.items()},
                'zombies': {zid: {'x': z.x,'y': z.y,'hp': z.hp,'isFacingRight': z.isFacingRight,
                                  'isAttacking': z.attackTimer>0} for zid,z in room.zombies.items()},
                'wave': room.wave
            }
...
def on_join(data):
    ...
    if code not in active_rooms:
        emit('error', {'msg': 'Комнаты не существует!'}); return
    room = active_rooms[code]
    max_players = 2 if room.mode=="PvP" else 4
    if len(room.players) >= max_players:
        emit('error', {'msg': 'Арена заполнена!'}); return
    join_room(code)
    coins = get_player_coins(username)
...
def handle_input(data):
    ...
    if sid in connected_players:
        ...
        if code in active_rooms and sid in (room:=active_rooms[code]).players:
            p = room.players[sid]
...
def on_disconnect():
    ...
    if sid in connected_players:
        ...
        if code in active_rooms:
            room = active_rooms[code]
            if sid in room.players: del room.players[sid]
            if len(room.players)==0: del active_rooms[code]
            else: socketio.emit('opponent_left', room=code)
